...
abstract interface class TableInterface {
...
class CriterionAll extends TableInterface {
  const CriterionAll({
    required super.type,
...
class CriterionClub extends TableInterface {
  const CriterionClub({
    required super.type,
    required super.target,
    super.note,
...
class CriterionPersonal extends TableInterface {
...
class AllocationAll extends TableInterface {
  const AllocationAll({
    required super.type,
    required super.target,
    super.note,
  ...
  List<Widget> rowColumnWidget({required divider}) {
    return [
      Expanded(flex: 4, child: _container(_indexText(type))),
      divider,
      Expanded(flex: 4, child: _container(_indexText(target))),
      divider,
      Expanded(flex: 7, child: _container(_indexText(note))),
    ];
  }
}
...
Widget _indexText(String text, {Color? color}) =>
    IndexMicroText(height: 1.2, textAlign: TextAlign.center, text, textColor: color,defaultScale: true,);
